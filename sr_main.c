...
#define DEFAULT_TIMEOUT_TCP_EST 7440
#define DEFAULT_TIMEOUT_TCP_TRANS 300
...
int main(int argc, char **argv)
{
    ...
    unsigned int timeout_tcp_est = DEFAULT_TIMEOUT_TCP_EST;
    unsigned int timeout_tcp_trans = DEFAULT_TIMEOUT_TCP_TRANS;
    ...
    while ((c = getopt(argc, argv, "hs:v:p:u:t:r:l:T:I:E:R:n")) != EOF)
    {
        switch (c)
        {
            ...
            case 'n':
                nat_enabled = 1;
                fprintf(stderr, "Got -n flag.\n");
                break;
            case 'I':
                /*if ((timeout_tcp_icmp= atoi((char *) optarg)) <= DEFAULT_TIMEOUT_ICMP) {
                  fprintf(stderr,
                          "Timeout must be at least 1 minute. Using default value 60s.\n");
                  timeout_icmp = DEFAULT_TIMEOUT_ICMP;
                }*/
                timeout_icmp = atoi((char *) optarg);
                break;
            case 'E':
                /*if ((timeout_tcp_est= atoi((char *) optarg)) <= DEFAULT_TIMEOUT_TCP_EST) {
                  fprintf(stderr,
                          "Timeout must be at least 2 hours and 4 minutes. Using default value 7440s.\n");
                  timeout_tcp_est = DEFAULT_TIMEOUT_TCP_EST;
                }*/
                timeout_tcp_est= atoi((char *) optarg);
                break;
            case'R':
                /*if ((timeout_tcp_trans= atoi((char *) optarg)) <= 240){
                  fprintf(stderr,
                          "Timeout must be at least 4 minutes. Using default value 300s.\n");
                  timeout_tcp_trans = DEFAULT_TIMEOUT_TCP_TRANS;
                }*/
                timeout_tcp_trans = atoi((char *) optarg);
    ...
    if (nat_enabled) {
      fprintf(stderr, "initializing nat.\n");
      sr.nat = (struct sr_nat *) malloc(sizeof(struct sr_nat));
      sr_nat_init(sr.nat);

      sr.nat->sr = &sr;

      sr.nat->timeout_icmp = timeout_icmp;
      sr.nat->timeout_tcp_est = timeout_tcp_est;
      sr.nat->timeout_tcp_trans = timeout_tcp_trans;
    }
